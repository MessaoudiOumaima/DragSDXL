...
class DragPipeline(StableDiffusionPipeline):
    ...
    def forward_unet_features(self, z, t, encoder_hidden_states, layer_idx=[0], interp_res=768):
    ...
    def invert(
        ...
        **kwds):
        ...
        print('input_ids',text_input.input_ids)
        #text_embeddings=text_embeddings[:,:640] #change added
        ...
        for i, t in enumerate(tqdm(reversed(self.scheduler.timesteps), desc="DDIM Inversion")):
            ...
            #time_ids=self.scheduler.timesteps.expand(-1,batch_size).to('cuda')
            #batch size should be 1
            time_ids = self.scheduler.timesteps[None, ...].to('cuda')
            t_emb = self.time_proj(time_ids)
            t_emb = self.time_embedding(t_emb, time_ids)

            print('ids size', time_ids.size())
