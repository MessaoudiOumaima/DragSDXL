...
from DragSDXLPipeline import DragXLPipeline
...
#model = StableDiffusionXLPipeline.from_pretrained("stabilityai/stable-diffusion-xl-base-1.0", torch_dtype=torch.float16, use_safetensors=True, variant="fp16")
#model.to("cuda")
...
model = DragXLPipeline.from_pretrained("stabilityai/stable-diffusion-xl-base-1.0", scheduler=scheduler,).to(device)
#diffusion_model = DragPipeline.from_pretrained("runwayml/stable-diffusion-v1-5", scheduler=scheduler).to(device)#"runwayml/stable-diffusion-v1-5"# #
#model= DragSDXLPipeline.from_pretrained("stabilityai/stable-diffusion-xl-base-1.0", scheduler=scheduler).to(device)
# call this function to override unet forward function,
# so that intermediate features are returned after forward
#model.unet = torch.compile(model.unet, mode="reduce-overhead", fullgraph=True)
#diffusion_model.unet=  torch.compile(diffusion_model.unet, mode="reduce-overhead", fullgraph=True)
#print('unet', diffusion_model.unet)
#model.modify_unet_forward()



model.load_lora_weights('/home/uma/projects/stable_diffusion/out_xl/pytorch_lora_weights.safetensors')
...
def inference(source_image,
    ...
    #accelerate_seed = random.randint(1, 60)
    ...
    args.unet_feature_idx = [0]

    # args.sup_res = sup_res
    args.sup_res = 256 #changed from 256 to 768
    ...
    # set lora
    #lora_path="/home/uma/lora/lora_ckpt/anatomy"

    lora_path= "/home/uma/projects/stable_diffusion/out_xl/pytorch_lora_weights.safetensors"
    ...
    invert_code = model.invert(image=source_image,
                               prompt=prompt,
    ...
    updated_init_code, updated_text_emb = drag_diffusion_update(model, init_code, t,
        handle_points, target_points, mask, accelerator, args)
